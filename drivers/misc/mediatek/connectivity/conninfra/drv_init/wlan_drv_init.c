...
{
	pr_debug("no impl. mtk_wcn_wlan_gen4_init\n");
	return 0;
}

int __attribute__((weak)) mtk_wcn_wmt_wifi_init(void)
{
	pr_debug("no impl. mtk_wcn_wmt_wifi_init");
	return 0;
}
...
int do_wlan_drv_init(int chip_id)
{
	...
	pr_debug("Start to do wlan module init 0x%x\n", chip_id);

	/* WMT-WIFI char dev init */
	ret = mtk_wcn_wmt_wifi_init();
	pr_debug("WMT-WIFI char dev init, ret:%d\n", ret);
	i_ret += ret;

	/* WLAN driver init */
	ret = mtk_wcn_wlan_gen4_init();
	pr_debug("WLAN-GEN4 driver init, ret:%d\n", ret);

	i_ret += ret;

	pr_debug("Finish wlan module init\n");
